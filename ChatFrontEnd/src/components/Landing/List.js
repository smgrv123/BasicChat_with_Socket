      ...
      <View style={{flexDirection:"column",flex:1}} >
        <View style={styles.textContainer}>
          ...
          <Text
            style={
              styles.details
            }>{`${item.age} | ${item.height} | ${item.country}`}</Text>
          <ListObj
            item={item.education}
            type={'entypo'}
            name="graduation-cap"
          />
          <ListObj item={item.job} type={'foundation'} name="shopping-bag" />
          <ListObj item={item.status} type={'ant-design'} name="heart" />
        </View>
